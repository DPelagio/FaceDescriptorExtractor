...
int main()
    {
      ...
      cv::Mat image2 = imread("../johns/ab.jpg", CV_LOAD_IMAGE_COLOR);//cargamos dos imágenes para demostrar cómo se utilizan todas nuestras funciones
      cv::imshow("Imagen Dada", image2);//Aquí dejamos las imágenes en pantalla para demostración visual
      cv::imshow("Imagen Dada 2", image);
      ...
      matrix<float,0,1> vector1 = fr.obtenerDescriptorVectorial(image);//Nuestra función obtenerDescripcionVectorial recibe un Mat a color de una imagen de 150x150
      matrix<float,0,1> vector2 = fr.obtenerDescriptorVectorial(image2);//Nuestra función regresa una matrix<float,0,1> la cual pertenece a dlib
